...
// kontrolo správných čísel
    ...
    }).filter(data => data !== null);  // odstraní neplatné řádky
    console.log('GPS Data:', this.gpsData);  // Výpis GPS dat do konzole
  }
  ...
  addGpsDataToMap(): void {
    ...
    }).filter(latlng => latlng !== null);  // odstraní neplatné body
  ...
  findClosestBts(gpsTimestamp: Date): BtsData | null {
    ...
    this.btsData.forEach(bts => {
      ...
      // Pokud je rozdíl menší, než dosud nalezený, aktualizace closestBts
  ...
  saveData(): void {
    this.dataService.saveData(this.btsData, this.gpsData).subscribe(
      (response) => {
        console.log('Data úspěšně uložena v databázi', response);
        alert('Data úspěšně uložena v databázi!');
      },
      (error) => {
        console.error('Error ukládání dat se nezdařilo', error);
        alert('Error ukládání dat se nezdařilo.');
      }
    );
  }
}
